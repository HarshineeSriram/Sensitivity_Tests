...
class LimeBase(PerturbationAttribution):
    ...
    def _evaluate_batch(
        ...
        model_out = torch.max(model_out).int()
        if isinstance(model_out, Tensor):
            # print("Model output numel: ", model_out.numel()) -> This prints out 1000
            # print("\n Current model inputs length: ", len(curr_model_inputs)) -> This prints out 1
...
def featuremask(img):
    ...
    return torch.from_numpy(superpixels), num_superpixels
