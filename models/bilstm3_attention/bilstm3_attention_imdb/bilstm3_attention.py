...
class BiLSTM3Attention(nn.Module):
    def __init__(self, batch_size, output_size, hidden_size, vocab_size, embedding_length, weights):
        super(BiLSTM3Attention, self).__init__()
    ...
    def forward(self, input_sentences, batch_size=None):
        ...
        if batch_size is None:
            h_0 = Variable(torch.zeros(2 * self.num_layers, self.batch_size, self.hidden_size).cuda())
            c_0 = Variable(torch.zeros(2 * self.num_layers, self.batch_size, self.hidden_size).cuda())
        else:
